...
def create(
    ...
    if start_datetime is None or end_datetime is None:
        feature_pipeline_metadata = utils.load_json("feature_pipeline_metadata.json")
        start_datetime = feature_pipeline_metadata["datetime_start"]
        end_datetime = feature_pipeline_metadata["datetime_end"]
    ...
    try:
        feature_views = fs.get_feature_views(name="crypto_forcasting_view")
    except hsfs.client.exceptions.RestAPIError:
        logger.info("No feature views found for crypto_forcasting_view.")
    ...
    energy_consumption_fg = fs.get_feature_group(
        "btc_forcasting", version=feature_group_version
    )
    ds_query = energy_consumption_fg.select_all()
    feature_view = fs.create_feature_view(
        name="crypto_forcasting_view",
    ...
    feature_view.create_training_data(
        description="btc forcasting training dataset",
        data_format="csv",
